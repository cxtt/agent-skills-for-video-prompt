...
4. 技能规则：录入触发关键词及对应功能，参考`trigger-words.md`；
5. 保存后，输入「引导」即可开启分步创作。

### 3. 百度文心一言
1. 打开文心一言，进入「定制→定制问答→新增问答」；
2. 批量录入触发关键词作为「问题」，对应功能/流程作为「答案」；
...
2. **Configure**页：
   ...
   - Description：高效生成合规可过审的短视频提示词，支持关键词触发、对话引导、视频比例适配；
...
2. 配置Assistant：
   ...
   - Instructions：录入7大核心技能的流程和过审检测规则；
   - Tools：无需添加工具，仅保留「Code Interpreter」（可选）；
3. 将触发关键词封装为**API调用参数**，对应不同的技能执行逻辑；
4. 调用Assistant API，传入触发关键词/用户需求，即可自动返回结果。

## 三、开源Agent框架（轻量开发，技术爱好者/团队）
### 1. LangChain（主流推荐）
1. 将7大核心技能封装为**LangChain Tools**，每个Skill对应一个Tool函数；
2. 将触发关键词设为**Tool触发词**，通过Prompt工程实现关键词匹配；
...
3. 将7大核心技能封装为**Functions**，实现技能的自动化调用；
